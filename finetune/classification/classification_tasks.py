...
class SingleOutputTask(task.Task):
  ...
  def featurize(self, example: InputExample, is_training, log=False):
    ...
    class_weights = [positive / (self.config.max_seq_length / 8), (self.config.max_seq_length - positive) / (self.config.max_seq_length / 8)]
    tokens.append("[SEP]")
    segment_ids.append(0)
    cls_ids.append(0)
    label_ids.append(0)
...
class ClassificationTask(SingleOutputTask):
  ...
  def get_feature_specs(self):
    return [feature_spec.FeatureSpec(self.name + "_eid", []),
            ...
            feature_spec.FeatureSpec(self.name + "_class_weights", [2], is_int_feature=False)]
  ...
  def get_prediction_module(self, bert_model, features, is_training,
    ...
    weights_expand = tf.expand_dims(class_weights, 1)
    tiled_weights = tf.tile(weights_expand, d2)
    utils.log(tiled_weights)
    weights = tf.reduce_sum(tf.cast(tiled_weights, tf.float32) * onehot_labels, axis=-1)
    utils.log(weights)
